...
@push('meta')
<title>{{ $item ? 'Update' : 'Add' }} Gallery | {{ config('app.name') }}</title>
<meta content="{{ $item ? 'Update' : 'Add' }} Gallery" name="description" />
...
<div class="app__slide-wrapper">
        ...
        <div class="breadcrumb__wrapper mb-25">
            <nav>
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ $item ? 'Update' : 'Add' }} Gallery</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-xxl-12 col-xl-12 col-lg-12">
            <form class="card__wrapper" action="{{ route('gallery.store') }}" method="post" enctype="multipart/form-data" onsubmit="return updateProfileimgs(this)">
                    ...
                    <div class="col-lg-12">
                        <div class="from__input-box">
                            <div class="form__input-title">
                                <label for="image">Gallery Image<span>*</span></label>
                            </div>
                            <div class="form__input">
                                <input type="file" id="image" name="image" class="form-control" accept="image/*" onchange="previewGallery(this)" {{ $item ? 'disabled' : '' }} />

                                <!-- Existing Image -->
                                <div class="mt-2">
                                    <img id="galleryPreview" src="{{ isset($item->image) ? asset('storage/' . $item->image) : '' }}" alt="Gallery Preview" style="height:100px; {{ isset($item->image) ? '' : 'display:none;' }}">
                                </div>
